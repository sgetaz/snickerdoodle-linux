...
static struct sx150x_platform_data *sx150x_of_probe(struct device *dev)
{
	...
	pdata->oscio_is_gpo = of_property_read_bool(dev->of_node, "oscio-is-gpio");

	pdata->io_pullup_ena = sx150x_of_probe_pins(dev, "pull-up-ports");

	pdata->io_pulldn_ena = sx150x_of_probe_pins(dev, "pull-down-ports");

	pdata->io_polarity = sx150x_of_probe_pins(dev, "polarity-invert-ports");

	pdata->irq_summary = of_irq_get(dev->of_node, 0);

	/* irq_base is not needed with OF */
	pdata->reset_during_probe = of_property_read_bool(dev->of_node, "probe-reset");
